...
public class CalendarCalculationsTest {
    ...
    public void firstDayOfWeek() {
        ...
        assertEquals(Calendar.MONDAY, FallenCalendarView.firstDayOfYear(calendar2018));

        Calendar calendar2017 = Calendar.getInstance();
        calendar2017.set(2017, 12-1, 5);
        assertEquals(Calendar.SUNDAY, FallenCalendarView.firstDayOfYear(calendar2017));

        Calendar calendar2017_1 = Calendar.getInstance();
        calendar2017_1.set(2017, 10-1, 1);
        assertEquals(Calendar.SUNDAY, FallenCalendarView.firstDayOfYear(calendar2017_1));
        ...
        assertEquals(Calendar.SUNDAY, FallenCalendarView.firstDayOfYear(calendar2017_2));

        for (int year = 1992; year < 2007; ++year) {
            Calendar firstOfJan = Calendar.getInstance();
            firstOfJan.set(year, 0, 1);
            assertEquals(firstOfJan.get(Calendar.DAY_OF_WEEK), FallenCalendarView.firstDayOfYear(firstOfJan));
            for (int month = 2; month < 12; month += 2) {
                for (int day = 3; day < 29; day += 4) {
                    Calendar calendar = Calendar.getInstance();
                    calendar.set(year, month, day);
                    calendar.setFirstDayOfWeek(year*month*day % 7 + 1);
                    assertEquals(firstOfJan.get(Calendar.DAY_OF_WEEK), FallenCalendarView.firstDayOfYear(calendar));
                }
            }
        }
    }
